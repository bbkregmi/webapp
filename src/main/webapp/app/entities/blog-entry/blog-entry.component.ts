...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.blogEntryService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<BlogEntry[]>) => this.onSuccess(res.body, res.headers),
                    (res: HttpErrorResponse) => this.onError(res.message)
                );
            return;
        }
        this.blogEntryService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<BlogEntry[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/blog-entry'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
